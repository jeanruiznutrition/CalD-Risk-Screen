...
### B1d. Dos puntajes óseos y tiempo de exposición

**Dos puntajes, no uno.** El motor calcula ahora un puntaje óseo SIMPLE
—solo calcio absorbible y vitamina D, máximo 4 puntos— junto al completo,
que añade edad, sexo, actividad física con resistencia, tabaquismo,
alcohol y el umbral de EPIC-Oxford. Los dos se muestran y los dos se
exportan.

La razón es que la pregunta de si esos componentes hacen falta PARA ESTE
INSTRUMENTO es empírica, no doctrinal. Son determinantes documentados de
la masa ósea, pero un instrumento más corto que prediga igual de bien es
un instrumento mejor. Los dos se contrastan contra densitometría en la
misma muestra y ambos escenarios dan resultado publicable: si el simple
no pierde capacidad predictiva, la versión definitiva es la corta; si el
completo gana, se publica con la evidencia de por qué.

La suite fija la propiedad que hace interpretable esa comparación: **el
puntaje simple no se mueve con las variables que no usa.** Si cambiara con
la edad o el tabaquismo, la diferencia entre los dos puntajes no aislaría
la contribución de esos componentes. El puntaje simple declara además su
lista de exclusiones, para que el análisis no tenga que leer el código.

Proteína y actividad física se siguen midiendo siempre: sin esos datos la
comparación es imposible, y en EPIC-Oxford el exceso de riesgo de fractura
en veganos se atenuaba cuando la ingesta de calcio **y proteína** era
adecuada.

**Tiempo de exposición, no solo presencia.** Tres campos nuevos: meses
suplementando calcio, meses suplementando vitamina D y meses entrenando
con regularidad. Hasta ahora la herramienta registraba si el participante
suplementa, pero no desde cuándo, y eso confunde dos situaciones muy
distintas: la 25-hidroxivitamina D sérica tarda del orden de tres meses en
alcanzar el estado estacionario, así que por debajo de ese tiempo el valor
sérico **no refleja todavía la dosis que se está tomando**; y la respuesta
ósea al ejercicio con resistencia se mide también en meses. Sin esta
variable, quien empezó ayer y quien lleva cinco años entraban al análisis
como iguales.

**Pantalla de resultados emparejada por nutriente.** El riesgo de
deficiencia de calcio pasó a estar inmediatamente debajo de las cifras de
calcio —ingerido y absorbido—, en vez de separado por dos bloques
intermedios. La vitamina D ya tenía su riesgo junto a sus cifras, con la
síntesis cutánea y la ingesta mostradas por separado y sin sumarse, porque
la ingesta de referencia se definió suponiendo exposición solar mínima.
...
Ahora se documentan **142 campos** —130 de la herramienta y 12 columnas
