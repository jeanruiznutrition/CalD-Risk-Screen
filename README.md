...
Sello del motor: `CARDA-v6.0+564fedd1`
...
- **Dos puntajes de riesgo óseo.** Uno con solo calcio y vitamina D, otro
  añadiendo edad, sexo, actividad física, tabaco y alcohol. Ambos se
  contrastan contra densitometría en el estudio de validación para
  determinar si los componentes no nutricionales aportan capacidad
  predictiva. Si no la aportan, la versión definitiva será la corta.
- **Tiempo de exposición.** Meses suplementando calcio y vitamina D, y
  meses entrenando con regularidad. La 25-hidroxivitamina D tarda unos
  tres meses en alcanzar el estado estacionario, así que sin esa variable
  un valor sérico bajo en alguien que empezó a suplementar hace tres
  semanas se interpretaría mal.
- **Recomendaciones accionables.** Deriva al profesional que corresponde
  —y a un nutricionista con formación en nutrición basada en plantas
  cuando el patrón lo es—, sugiere 25-hidroxivitamina D sérica cuando hace
  falta, y densitometría según los criterios de guía por edad y sexo más el
  riesgo calculado. **No** recomienda densitometría por patrón dietético: en
  EPIC-Oxford el exceso de riesgo en veganos se atenuaba con ingesta
  adecuada, así que el riesgo lo determina no cubrir el requerimiento.
- **Distribución de las fuentes.** Dice en cuántos días de la semana el
  patrón declarado alcanzaría la meta y si el calcio está concentrado en
  pocas comidas, con el margen que se recuperaría repartiéndolo. Es la
  única recomendación que no cuesta dinero ni cambia lo que se come.
- **Pestañas de Laboratorio y Bibliografía.** Interpretación de los
  analitos y de los patrones que forman entre ellos; y las 53 fuentes del
  modelo, generadas desde el registro de parámetros para que no puedan
  quedar desincronizadas con el código.
...
- **Diccionario de datos** de 142 campos, exportable en CSV y en formato
  de importación de REDCap, más exportación en formato largo.
- **Registro de parámetros con grado de evidencia.** Las 55 constantes
  del modelo declaran su fuente y su grado; los 10 parámetros heurísticos
