...
// ------------------------------------------------------------
// (RETIRADO) RIESGO DE SARCOPENIA
// ------------------------------------------------------------
// El SARC-F y el SARC-CalF se retiraron de la herramienta en la v6.0.
// Son instrumentos validados por terceros, no son aporte de este
// motor, y no sirven a su propósito. Además el SARC-F se derivó en
// adultos mayores, de modo que mostrarlo a participantes jóvenes lo
// usaba fuera de su población de derivación: el mismo error que el
// motor ya evita al negarse a calcular el ORAI en varones y por debajo
// de los 45 años.
//
// La PROTEÍNA no se retiró y no debe retirarse: no es sarcopenia. La
// calidad proteica es central en una dieta basada en plantas y la
// proteína es determinante de la masa ósea.
...
// ------------------------------------------------------------
// 17. DISTRIBUCIÓN DE LAS FUENTES A LO LARGO DE LA SEMANA
// ------------------------------------------------------------
// POR QUÉ ESTA MÉTRICA, Y POR QUÉ NO SE LLAMA «DÍAS CUMPLIDOS»
//
// La semana virtual NO es un registro de los días del participante: es
// una reconstrucción que el motor hace repartiendo las frecuencias
// declaradas. Si alguien declara tofu tres veces por semana, el motor lo
// coloca en tres días, pero no son *esos* tres días. Llamar «días
// cumplidos» al resultado prometería una medición que no existe.
//
// Lo que sí se puede afirmar es cuántos días por semana el patrón
// declarado ALCANZARÍA la meta, y si la distribución es eficiente. Y eso
// tiene valor educativo directo, que es para lo que se pidió: por la
// curva de saturación de Heaney, la misma cantidad de calcio repartida
// entre varias comidas se absorbe más que concentrada en una. Un
// participante puede mejorar su calcio absorbido sin cambiar nada de lo
// que come, solo repartiéndolo mejor. Esta métrica es la que permite
// enseñárselo.
const evaluarDistribucion = (resultadosCalcio) => {
    if (!resultadosCalcio || !resultadosCalcio.semanaVirtual) return null;

    const meta = resultadosCalcio.metaAbsorbidaDiaria ||
                 (resultadosCalcio.referencia && resultadosCalcio.referencia.metaAbsorbida) || 0;
    const semana = resultadosCalcio.semanaVirtual;

    let diasQueAlcanzan = 0;
    let comidasConAporte = 0;
    let comidasTotales = 0;
    let mejorComida = 0;
    let absorbidoSemana = 0;
    const porDia = [];

    semana.forEach(dia => {
        let absorbidoDia = 0;
        dia.forEach(comida => {
            const abs = Number(comida.totalAbsorbido) || 0;
            absorbidoDia += abs;
            comidasTotales += 1;
            if (abs > 0) comidasConAporte += 1;
            if (abs > mejorComida) mejorComida = abs;
        });
        absorbidoSemana += absorbidoDia;
        porDia.push(Math.round(absorbidoDia * 10) / 10);
        if (meta > 0 && absorbidoDia >= meta) diasQueAlcanzan += 1;
    });

    // Concentración: qué fracción de todo el calcio absorbido de la semana
    // cae en la comida de mayor aporte. Un valor alto significa que el
    // calcio está apiñado, y apiñarlo cuesta absorción por saturación.
    const concentracion = absorbidoSemana > 0 ? mejorComida / absorbidoSemana : 0;
    const comidasPorDiaConAporte = comidasTotales > 0 ? (comidasConAporte / comidasTotales) * 3 : 0;

    // El veredicto es de EDUCACIÓN, no de riesgo: dice si conviene
    // redistribuir, que es una acción que el participante puede tomar sin
    // cambiar su presupuesto ni su patrón alimentario.
    let veredicto, colorKey;
    if (diasQueAlcanzan >= 6 && comidasPorDiaConAporte >= 2) {
        veredicto = 'adecuada'; colorKey = 'emerald';
    } else if (diasQueAlcanzan >= 4 || comidasPorDiaConAporte >= 2) {
        veredicto = 'ajustable'; colorKey = 'amber';
    } else {
        veredicto = 'concentrada'; colorKey = 'rose';
    }

    // Margen recuperable por redistribución: si el calcio de la comida de
    // mayor carga se repartiera, la absorción fraccional de esa carga
    // subiría. Se estima con la propia curva, sin cambiar nada de lo que
    // el participante come.
    let margenRedistribucion = null;
    if (mejorComida > 0 && typeof absorcionFraccionalPorCarga === 'function') {
        const diasConAporte = porDia.filter(d => d > 0).length || 1;
        const cargaMedia = absorbidoSemana / (diasConAporte * 3);
        if (cargaMedia > 0) {
            const faConcentrada = absorcionFraccionalPorCarga(mejorComida);
            const faRepartida = absorcionFraccionalPorCarga(cargaMedia);
            margenRedistribucion = Math.max(0, Math.round((faRepartida - faConcentrada) * 1000) / 10);
        }
    }
    ...
    return {
        diasQueAlcanzan,
        diasTotales: 7,
        metaDiaria: Math.round(meta * 10) / 10,
        comidasPorDiaConAporte: Math.round(comidasPorDiaConAporte * 10) / 10,
        concentracionMayorComida: Math.round(concentracion * 1000) / 10,
        margenRedistribucionPuntos: margenRedistribucion,
        absorbidoPorDia: porDia,
        veredicto,
        colorKey,
        // Recordatorio permanente: es una reconstrucción del patrón
        // declarado, no un registro de los días del participante.
        notaKey: 'dist_is_a_reconstruction'
    };
};


// ------------------------------------------------------------
// 18. RECOMENDACIONES: A QUIÉN DERIVAR Y QUÉ PRUEBAS SUGERIR
// ------------------------------------------------------------
// Emite tres cosas: a qué profesional acudir, si conviene solicitar
// 25-hidroxivitamina D sérica, y si corresponde densitometría.
//
// DERIVACIÓN SEGÚN EL PATRÓN ALIMENTARIO. Cuando el participante sigue un
// patrón flexitariano, vegetariano o vegano, la derivación lo dirige a un
// nutricionista con formación y experiencia en nutrición basada en
// plantas. La razón es de competencia: la biodisponibilidad del calcio
// vegetal, la fortificación de las bebidas vegetales y el manejo de la
// vitamina D sin fuentes animales requieren conocimiento específico. Se
// redacta como COMPETENCIA y no como credencial, porque no existe un
// registro formal de esa especialidad que la herramienta pueda invocar.
//
// DENSITOMETRÍA. Los criterios de edad y sexo son de guía publicada. NO
// hay criterio por patrón dietético, y la omisión es deliberada: en
// EPIC-Oxford el exceso de riesgo de fractura en veganos se atenuaba
// cuando la ingesta de calcio y proteína era adecuada, así que el riesgo
// lo determina no cubrir el requerimiento, no la etiqueta dietética. El
// patrón entra a través de la estimación de ingesta.
const generarRecomendaciones = ({
    edad, sexo, grupoEstudio,
    conducta,
    resultadoOseo,
    resultadoVitDDieta,
    resultadoSolar,
    panelBioquimico,
    fuma, alcoholFrecuente, fracturaPreviaFragilidad
}) => {
    const e = Number(edad) || 0;
    const esBasadaEnPlantas = ['Vegano', 'Vegetariano', 'Flexitariano'].indexOf(grupoEstudio) >= 0;
    const fraccionOseo = resultadoOseo && resultadoOseo.puntajeMaximo > 0
        ? resultadoOseo.puntaje / resultadoOseo.puntajeMaximo : 0;

    // --- A quién acudir ---
    const derivacion = {
        necesaria: !!(conducta && conducta.requiereDerivacion),
        profesionalKey: esBasadaEnPlantas ? 'refer_plant_based_dietitian' : 'refer_dietitian',
        esBasadaEnPlantas,
        // Un hallazgo bioquímico va al médico, no al nutricionista.
        requiereMedicoKey: (panelBioquimico &&
            (panelBioquimico.nivelDerivacion === 'derivacion' ||
             panelBioquimico.nivelDerivacion === 'derivacion_urgente'))
            ? 'refer_physician_biochem' : null
    };

    // --- 25-hidroxivitamina D sérica ---
    const yaTiene25OHD = !!(panelBioquimico && panelBioquimico.analitos &&
        panelBioquimico.analitos.vitd && panelBioquimico.analitos.vitd.valor !== null &&
        panelBioquimico.analitos.vitd.valor !== undefined);
    const motivos25OHD = [];
    if (!yaTiene25OHD) {
        // El caso en que la estimación NO puede concluir: la ingesta queda
        // bajo la referencia pero la síntesis cutánea es apreciable, y la
        // referencia se definió suponiendo exposición solar mínima.
        if (conducta && conducta.vitd && conducta.vitd.estado === 'indeterminado') {
            motivos25OHD.push('vitd_lab_indeterminate');
        }
        const razonD = conducta && conducta.vitd ? conducta.vitd.razon : null;
        if (razonD !== null && razonD < VITD_LAB_FRACCION_INGESTA * 100) {
            motivos25OHD.push('vitd_lab_low_intake');
        }
        // Dieta vegetariana o vegana sin fortificados ni suplemento: la
        // vía dietética no puede cubrirlo, así que el estado sérico es
        // la única forma de saber dónde está.
        if (esBasadaEnPlantas && conducta && conducta.vitd && conducta.vitd.sinAlimentosFortificados) {
            motivos25OHD.push('vitd_lab_no_fortified');
        }
        if (fraccionOseo >= VITD_LAB_FRACCION_RIESGO_OSEO) {
            motivos25OHD.push('vitd_lab_bone_risk');
        }
    }
    ...
    if (esMujer && e >= DXA_EDAD_MUJER) motivosDXA.push('dxa_age_women');
    if (!esMujer && e >= DXA_EDAD_VARON) motivosDXA.push('dxa_age_men');
    if (e >= DXA_EDAD_CON_FACTORES && e < (esMujer ? DXA_EDAD_MUJER : DXA_EDAD_VARON)) {
        const factores = [];
        if (fraccionOseo >= DXA_FRACCION_RIESGO) factores.push('riesgo');
        if (fuma) factores.push('tabaco');
        if (alcoholFrecuente) factores.push('alcohol');
        if (factores.length) motivosDXA.push('dxa_age_with_risk');
    }
    if (fracturaPreviaFragilidad) motivosDXA.push('dxa_previous_fracture');
    if (panelBioquimico && panelBioquimico.nivelDerivacion === 'derivacion_urgente') {
        motivosDXA.push('dxa_biochemical_finding');
    }

    return {
        derivacion,
        solicitar25OHD: motivos25OHD.length > 0,
        motivos25OHD,
        yaTiene25OHD,
        solicitarDXA: motivosDXA.length > 0,
        motivosDXA,
        // Se declara explícitamente que el patrón dietético no es criterio,
        // porque es una omisión deliberada y un revisor va a preguntarlo.
        notaPatronDietetico: 'dxa_no_diet_criterion',
        descargoKey: 'recommend_professional_decides'
    };
};


// ------------------------------------------------------------
// 19. PUNTAJE ÓSEO SIMPLE: SOLO CALCIO Y VITAMINA D
// ------------------------------------------------------------
// POR QUÉ EXISTEN DOS PUNTAJES
//
// El puntaje completo incorpora edad, sexo, actividad física con
// resistencia, tabaquismo, alcohol y el umbral de EPIC-Oxford. Todos son
// determinantes documentados de la masa ósea, pero la pregunta de si
// hacen falta PARA ESTE INSTRUMENTO es empírica, no doctrinal: un
// instrumento más corto que prediga igual de bien es mejor instrumento.
//
// Así que el motor calcula los dos y el estudio decide. El simple usa
// exclusivamente los dos componentes que la herramienta estima con su
// propio modelo —calcio absorbible y vitamina D por las tres vías—; el
// completo añade el resto. Ambos se contrastan contra densitometría en
// la misma muestra, y los dos escenarios dan un resultado publicable:
// si el simple no pierde capacidad predictiva, se publica el simple; si
// el completo gana, se publica con la evidencia de por qué.
//
// Proteína y actividad física se SIGUEN midiendo siempre, porque sin
// esos datos la comparación es imposible y porque en EPIC-Oxford el
// exceso de riesgo de fractura en veganos se atenuaba cuando la ingesta
// de calcio Y PROTEÍNA era adecuada.
const calcularRiesgoOseoSimple = ({ porcentajeCumplimientoCalcio, categoriaRiesgoSolar, categoriaVitDDieta }) => {
    const contribuciones = [];
    let puntaje = 0;
    const add = (pts, key) => { puntaje += pts; contribuciones.push({ key, pts }); };
    ...
    const puntajeMaximo = 4;
    const fraccion = puntaje / puntajeMaximo;
    let categoria, colorKey;
    if (fraccion >= 0.75) { categoria = 'alto'; colorKey = 'rose'; }
    else if (fraccion >= 0.40) { categoria = 'moderado'; colorKey = 'amber'; }
    else { categoria = 'bajo'; colorKey = 'emerald'; }

    return {
        puntaje, puntajeMaximo, fraccionDelMaximo: Math.round(fraccion * 1000) / 10,
        categoria, colorKey, contribuciones,
        // Se declara qué NO entra, para que la comparación del estudio sea
        // interpretable sin leer el código.
        excluye: ['edad', 'sexo', 'actividad_fisica', 'tabaco', 'alcohol', 'umbral_epic_oxford']
    };
};
