...
const calcularAdecuacionVitaminaD = (fuentes, suplementoVitD, edad) => {
    ...
    (fuentes || []).forEach(f => {
        if (!f.diasPorSemana || !f.vitDPorcion || !f.porcionesPorComida) return;
        const aporte = f.vitDPorcion * f.porcionesPorComida * (f.vecesPorDia || 1) * f.diasPorSemana;
        if (f.formaVitD === 'D2') mcgSemanalD2 += aporte;
        else mcgSemanalD3 += aporte;
    });
    ...
    if (suplementoVitD && suplementoVitD.mcgPorDia > 0 && suplementoVitD.diasPorSemana > 0) {
        ...
        if (suplementoVitD.forma === 'D2') mcgSemanalSuplD2 += total;
        else mcgSemanalSuplD3 += total;
    }

    const dietaBruta = (mcgSemanalD3 + mcgSemanalD2) / 7;
    const suplBruto = (mcgSemanalSuplD3 + mcgSemanalSuplD2) / 7;

    // Equivalentes de D3: ajustados por potencia relativa
    ...
    return {
        ...
        categoria,
        colorKey,
        // Señala si una parte relevante del aporte viene de D2 (menos potente)
...
// ------------------------------------------------------------
// 14. FRAX® — PREPARACIÓN DE DATOS (NO CÁLCULO)
// ------------------------------------------------------------
// Esta función NO calcula probabilidad de fractura. FRAX es un
// algoritmo propietario cuyos coeficientes no son públicos y cuya
// automatización requiere licencia de la Universidad de Sheffield.
// Lo que hace es empaquetar los factores de riesgo con las
// definiciones operativas de FRAX y construir el enlace a la
// calculadora oficial, donde el profesional obtiene la probabilidad.
const prepararDatosFRAX = ({ edad, sexo, pesoKg, tallaCm, factores, paisSustituto }) => {
    const e = Number(edad) || 0;
    const peso = Number(pesoKg) || 0;
    const talla = Number(tallaCm) || 0;
    const imc = (peso > 0 && talla > 0) ? Math.round((peso / Math.pow(talla / 100, 2)) * 10) / 10 : null;

    // FRAX está validado para 40-90 años. Fuera de ese rango no aplica.
    const edadEnRango = e >= 40 && e <= 90;

    const factoresPresentes = FACTORES_FRAX
        .filter(f => factores && factores[f.id])
        .map(f => f.nombreKey);

    const datosCompletos = e > 0 && peso > 0 && talla > 0;
    ...
    return {
        ...
        factores: factores || {},
        factoresPresentes,
        numeroFactores: factoresPresentes.length,
        paisSustituto: paisSustituto || 'ecuador',
        urlOficial: FRAX_URL_OFICIAL,
        // Nota que debe acompañar cualquier uso de estos datos
        requiereModeloSustituto: true
    };
};
