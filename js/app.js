...
function App() {
    ...
    const [perfil, setPerfil] = useState(() => restaurado('perfil', {
        ...
        tallaCm: 165,
        grupoEstudio: 'Vegano',
        // Antecedente de fractura por fragilidad: es el predictor más fuerte
        // de una nueva fractura y es criterio de densitometría a cualquier
        // edad. Hasta la v6.0 la herramienta no lo preguntaba.
        // Tiempo de exposición, no solo presencia. Un suplemento iniciado
        // hace tres semanas no ha podido modificar la masa ósea ni, en el
        // caso de la vitamina D, alcanzar el estado estacionario sérico
        // —que tarda del orden de tres meses—. Lo mismo con el
        // entrenamiento: la respuesta ósea al ejercicio con resistencia se
        // mide en meses. Sin esta variable, un participante que empezó
        // ayer y otro que lleva cinco años entran al análisis idénticos.
        mesesSuplementoCalcio: '',
        mesesSuplementoVitD: '',
        mesesEntrenamientoRegular: '',
        fracturaPreviaFragilidad: false,
        fuma: false,
    ...
    // El módulo de sarcopenia se retiró en la v6.0. El SARC-F y el
    // SARC-CalF son instrumentos validados por terceros, no son aporte de
    // esta herramienta, y el SARC-F se derivó en adultos mayores: mostrarlo
    // a participantes jóvenes lo usaba fuera de su población de derivación.
    // La proteína se conserva para todas las edades: no es sarcopenia.
    ...
    // Distribución de las fuentes a lo largo de la semana. Es una métrica
    // EDUCATIVA: por la curva de saturación, el mismo calcio repartido en
    // más comidas rinde más absorbido, así que el participante puede
    // mejorar sin cambiar lo que come.
    const distribucion = useMemo(
        () => evaluarDistribucion(resultadosCalcio),
        [resultadosCalcio]);

    // Recomendaciones: a qué profesional acudir, si pedir 25-hidroxivitamina
    // D y si corresponde densitometría. No hay criterio por patrón dietético.
    // Puntaje óseo SIMPLE: solo calcio y vitamina D. Existe para que el
    // estudio determine si los demás componentes hacen falta.
    const resultadoOseoSimple = useMemo(() => calcularRiesgoOseoSimple({
        porcentajeCumplimientoCalcio: razonSegunMarco,
        categoriaRiesgoSolar: resultadoSolarEstandar.categoriaRiesgo,
        categoriaVitDDieta: resultadoVitDDieta.categoria
    }), [razonSegunMarco, resultadoSolarEstandar.categoriaRiesgo, resultadoVitDDieta.categoria]);

    const recomendaciones = useMemo(() => generarRecomendaciones({
        edad: perfil.edad, sexo: perfil.sexo, grupoEstudio: perfil.grupoEstudio,
        conducta, resultadoOseo, resultadoVitDDieta,
        resultadoSolar: resultadoSolarEstandar, panelBioquimico,
        fuma: perfil.fuma, alcoholFrecuente: perfil.alcoholFrecuente,
        fracturaPreviaFragilidad: perfil.fracturaPreviaFragilidad
    }), [perfil.edad, perfil.sexo, perfil.grupoEstudio, conducta, resultadoOseo,
         resultadoVitDDieta, resultadoSolarEstandar, panelBioquimico,
         perfil.fuma, perfil.alcoholFrecuente, perfil.fracturaPreviaFragilidad]);
    ...
    useEffect(() => {
        guardarEstado({
            ...
            suplementosEntrenamiento, alimentos, alimentosExtra,
        ...
        alimentos, alimentosExtra, manualOverrides,
    ...
    const construirFilaParticipante = () => ({
        ...
        riesgoOseoSimplePuntaje: resultadoOseoSimple.puntaje,
        riesgoOseoSimpleFraccion: resultadoOseoSimple.fraccionDelMaximo,
        riesgoOseoSimpleCategoria: resultadoOseoSimple.categoria,
        mesesSuplementoCalcio: perfil.mesesSuplementoCalcio,
        mesesSuplementoVitD: perfil.mesesSuplementoVitD,
        mesesEntrenamientoRegular: perfil.mesesEntrenamientoRegular,
        distribucionDiasQueAlcanzan: distribucion ? distribucion.diasQueAlcanzan : '',
        distribucionVeredicto: distribucion ? distribucion.veredicto : '',
        distribucionConcentracionPct: distribucion ? distribucion.concentracionMayorComida : '',
        recomiendaDensitometria: recomendaciones.solicitarDXA ? 1 : 0,
        recomienda25OHD: recomendaciones.solicitar25OHD ? 1 : 0,
        derivacionProfesional: recomendaciones.derivacion.necesaria
            ? (recomendaciones.derivacion.esBasadaEnPlantas ? 'nutricionista_plantas' : 'nutricionista_general') : '',
        fracturaPreviaFragilidad: perfil.fracturaPreviaFragilidad ? 1 : 0,
            ...
            <main class="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
                ...
                <div class="max-w-5xl mx-auto mb-6 flex flex-wrap gap-1.5 no-print border-b border-slate-200 dark:border-slate-800 pb-3">
                    {[
                        ...
                        { id: 'laboratorio', icono: 'fa-vial-circle-check', clave: 'tab_lab' },
                        { id: 'bibliografia', icono: 'fa-quote-right', clave: 'tab_bibliography' },
                ...
                <div class="max-w-5xl mx-auto w-full">
                    ...
                    {vista === 'laboratorio' && <PanelLaboratorio t={t} />}
                    {vista === 'bibliografia' && <PanelBibliografia t={t} />}
                        ...
                        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="flex flex-col justify-end gap-1.5">
                                    <div>
                                        <label class="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">{t('profile_months_ca_supp')}</label>
                                        <input type="number" min="0" max="720" step="1" value={perfil.mesesSuplementoCalcio}
                                            onChange={(e) => handlePerfilChange('mesesSuplementoCalcio', limpiarNumero(e.target.value))}
                                            placeholder={t('profile_months_placeholder')}
                                            class="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg" />
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">{t('profile_months_vitd_supp')}</label>
                                        <input type="number" min="0" max="720" step="1" value={perfil.mesesSuplementoVitD}
                                            onChange={(e) => handlePerfilChange('mesesSuplementoVitD', limpiarNumero(e.target.value))}
                                            placeholder={t('profile_months_placeholder')}
                                            class="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg" />
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">{t('profile_months_training')}</label>
                                        <input type="number" min="0" max="720" step="1" value={perfil.mesesEntrenamientoRegular}
                                            onChange={(e) => handlePerfilChange('mesesEntrenamientoRegular', limpiarNumero(e.target.value))}
                                            placeholder={t('profile_months_placeholder')}
                                            class="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg" />
                                    </div>
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" checked={perfil.fracturaPreviaFragilidad} onChange={(e) => handlePerfilChange('fracturaPreviaFragilidad', e.target.checked)}
                                            class="w-4 h-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500" />
                                        <span class="text-xs text-slate-600 dark:text-slate-300">{t('profile_previous_fracture')}</span>
                                    </label>
                    ...
                    <section class="flex flex-col gap-6">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                ...
                                {/* 3. Distribución de las fuentes en la semana.
                                       No se llama «días cumplidos»: la semana es una
                                       reconstrucción del patrón declarado, no un registro
                                       de los días del participante. El encuadre es
                                       educativo — por la curva de saturación, la misma
                                       cantidad repartida en más comidas rinde más
                                       absorbido, así que el participante puede mejorar
                                       sin cambiar lo que come. */}
                                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                                    <div>
                                        ...
                                        {distribucion && (
                                            <div class="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                                                <p class={'text-xs font-bold ' + (
                                                    distribucion.colorKey === 'emerald' ? 'text-emerald-600 dark:text-emerald-400'
                                                    : distribucion.colorKey === 'amber' ? 'text-amber-600 dark:text-amber-400'
                                                    : 'text-rose-600 dark:text-rose-400')}>
                                                    <i class="fa-solid fa-chart-simple mr-1"></i>{t('dist_' + distribucion.veredicto)}
                                                </p>
                                                <p class="text-[10px] text-slate-500 mt-1 leading-relaxed">
                                                    {t('dist_concentration')}: <strong>{distribucion.concentracionMayorComida}%</strong>
                                                    {distribucion.margenRedistribucionPuntos !== null && distribucion.margenRedistribucionPuntos > 0 && (
                                                        <span> · {t('dist_margin')}: <strong>+{distribucion.margenRedistribucionPuntos} pp</strong></span>
                                                    )}
                                                </p>
                                                <p class="text-[10px] text-slate-400 mt-1.5 leading-relaxed italic">{t('dist_educational_note')}</p>
                                                <p class="text-[9px] text-slate-400 mt-1 leading-relaxed">{t('dist_is_a_reconstruction')}</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        {/* RIESGO DE DEFICIENCIA DE CALCIO — va inmediatamente
                            debajo de las cifras de calcio, emparejado con ellas:
                            primero cuánto entra y cuánto se absorbe, y después qué
                            significa. Antes quedaba separado por dos bloques. */}
                        ...
                        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                            <h3 class="font-bold text-sm text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-4 flex items-center gap-2">
                            {/* LOS DOS PUNTAJES. El estudio de validación determina si los
                                componentes que el simple no usa —edad, sexo, actividad física,
                                tabaco, alcohol, umbral de EPIC-Oxford— aportan capacidad
                                predictiva sobre la densitometría. Si no la aportan, la versión
                                publicable es la corta. */}
                            <div class="grid grid-cols-2 gap-3 mb-4">
                                <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800">
                                    <p class="text-[9px] font-bold uppercase tracking-wider text-slate-400">{t('bone_score_simple')}</p>
                                    <p class="text-2xl font-extrabold text-slate-800 dark:text-slate-100 mt-1">
                                        {resultadoOseoSimple.puntaje}<span class="text-[11px] font-semibold text-slate-400">/{resultadoOseoSimple.puntajeMaximo}</span>
                                    </p>
                                    <p class={'text-[10px] font-bold uppercase ' + (resultadoOseoSimple.colorKey === 'emerald' ? 'text-emerald-600' : resultadoOseoSimple.colorKey === 'amber' ? 'text-amber-600' : 'text-rose-600')}>
                                        {t('diag_bone_' + resultadoOseoSimple.categoria)}
                                    </p>
                                    <p class="text-[9px] text-slate-400 mt-1 leading-relaxed">{t('bone_score_simple_desc')}</p>
                                </div>
                                <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800">
                                    <p class="text-[9px] font-bold uppercase tracking-wider text-slate-400">{t('bone_score_full')}</p>
                                    <p class="text-2xl font-extrabold text-slate-800 dark:text-slate-100 mt-1">
                                        {resultadoOseo.puntaje}<span class="text-[11px] font-semibold text-slate-400">/{resultadoOseo.puntajeMaximo}</span>
                                    </p>
                                    <p class={'text-[10px] font-bold uppercase ' + (resultadoOseo.colorKey === 'emerald' ? 'text-emerald-600' : resultadoOseo.colorKey === 'amber' ? 'text-amber-600' : 'text-rose-600')}>
                                        {t('diag_bone_' + resultadoOseo.categoria)}
                                    </p>
                                    <p class="text-[9px] text-slate-400 mt-1 leading-relaxed">{t('bone_score_full_desc')}</p>
                                </div>
                            </div>
                            <p class="text-[10px] text-slate-500 dark:text-slate-400 leading-relaxed mb-4">{t('bone_score_compare_note')}</p>
                        ...
                        {/* RECOMENDACIONES */}
                        {/* Va después de los tres riesgos y antes de la semana virtual,
                            que es el orden que pidió el autor: primero qué hay, luego
                            qué hacer, y después el detalle que lo sustenta. */}
                        {(recomendaciones.derivacion.necesaria || recomendaciones.solicitar25OHD || recomendaciones.solicitarDXA) && (
                            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
                                <h3 class="font-bold text-sm text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-4 flex items-center gap-2">
                                    <i class="fa-solid fa-clipboard-check text-brand-600 dark:text-brand-400"></i> {t('recommend_title')}
                                </h3>
                                <div class="flex flex-col gap-3">
                                    {/* A quién acudir */}
                                    {recomendaciones.derivacion.necesaria && (
                                        <div class="p-4 rounded-xl border-l-4 border-brand-500 bg-brand-50/60 dark:bg-brand-950/25">
                                            <p class="text-[10px] font-bold uppercase tracking-wider text-brand-700 dark:text-brand-400 mb-1">
                                                <i class="fa-solid fa-user-doctor mr-1"></i>{t('conduct_refer_label')}
                                            </p>
                                            <p class="text-xs text-slate-700 dark:text-slate-300 leading-relaxed">{t(recomendaciones.derivacion.profesionalKey)}</p>
                                            {recomendaciones.derivacion.requiereMedicoKey && (
                                                <p class="text-[11px] text-rose-700 dark:text-rose-400 leading-relaxed mt-2 font-semibold">{t(recomendaciones.derivacion.requiereMedicoKey)}</p>
                                            )}
                                        </div>
                                    )}

                                    {/* 25-hidroxivitamina D */}
                                    {recomendaciones.solicitar25OHD && (
                                        <div class="p-4 rounded-xl border-l-4 border-amber-500 bg-amber-50/60 dark:bg-amber-950/25">
                                            <p class="text-[10px] font-bold uppercase tracking-wider text-amber-700 dark:text-amber-400 mb-1">
                                                <i class="fa-solid fa-vial mr-1"></i>{t('vitd_lab_title')}
                                            </p>
                                            <ul class="flex flex-col gap-1.5 mt-1">
                                                {recomendaciones.motivos25OHD.map(k => (
                                                    <li key={k} class="text-[11px] text-slate-700 dark:text-slate-300 leading-relaxed flex gap-2">
                                                        <i class="fa-solid fa-angle-right text-amber-500 mt-1 text-[9px]"></i><span>{t(k)}</span>
                                                    </li>
                                                ))}
                                            </ul>
                                        </div>
                                    ...
                                    {/* Densitometría */}
                                    {recomendaciones.solicitarDXA && (
                                        <div class="p-4 rounded-xl border-l-4 border-sky-500 bg-sky-50/60 dark:bg-sky-950/25">
                                            <p class="text-[10px] font-bold uppercase tracking-wider text-sky-700 dark:text-sky-400 mb-1">
                                                <i class="fa-solid fa-bone mr-1"></i>{t('dxa_title')}
                                            </p>
                                            <ul class="flex flex-col gap-1.5 mt-1">
                                                {recomendaciones.motivosDXA.map(k => (
                                                    <li key={k} class="text-[11px] text-slate-700 dark:text-slate-300 leading-relaxed flex gap-2">
                                                        <i class="fa-solid fa-angle-right text-sky-500 mt-1 text-[9px]"></i><span>{t(k)}</span>
                                                    </li>
                                                ))}
                                            </ul>
                                        </div>
                                ...
                                {/* El patrón dietético NO es criterio: se declara, porque es
                                    una omisión deliberada que un revisor va a preguntar. */}
                                <p class="text-[10px] text-slate-500 dark:text-slate-400 leading-relaxed mt-4 pt-3 border-t border-slate-200 dark:border-slate-800">
                                    <i class="fa-solid fa-circle-info mr-1"></i>{t(recomendaciones.notaPatronDietetico)}
                                </p>
                                <p class="text-[10px] text-slate-500 dark:text-slate-400 leading-relaxed mt-2">{t(recomendaciones.descargoKey)}</p>
                            </div>
                        )}
                        ...
                        {/* Va al final del recorrido, después de la semana virtual, que
                            es el orden que pidió el autor. Es un resumen: la versión
                            detallada con ecuaciones vive en la pestaña de Metodología, y
                            las fuentes en la de Bibliografía. Antes había aquí dos
                            bloques —metodología y bibliografía— que se disolvieron al
                            darles pestaña propia. */}
                        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
                            <h3 class="font-bold text-sm text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-1 flex items-center gap-2">
                                <i class="fa-solid fa-gears text-slate-500"></i> {t('how_it_works_title')}
                            </h3>
                            <p class="text-[11px] text-slate-500 dark:text-slate-400 mb-4 leading-relaxed">{t('how_intro')}</p>
                            <ol class="flex flex-col gap-3">
                                {[1, 2, 3, 4, 5, 6].map(n => (
                                    <li key={n} class="flex gap-3">
                                        <span class="shrink-0 w-6 h-6 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[11px] font-extrabold flex items-center justify-center">{n}</span>
                                        <div class="min-w-0">
                                            <p class="text-xs font-bold text-slate-800 dark:text-slate-200">{t('how_step_' + n + '_title')}</p>
                                            <p class="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed mt-0.5">{t('how_step_' + n + '_desc')}</p>
                                        </div>
                                    </li>
                                ))}
                            </ol>
                            <div class="mt-4 pt-3 border-t border-slate-200 dark:border-slate-800">
                                <p class="text-[11px] font-bold text-slate-700 dark:text-slate-300">{t('how_limits_title')}</p>
                                <p class="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed mt-1">{t('how_limits_desc')}</p>
                            </div>
                            <div class="mt-4 flex flex-wrap gap-2">
                                <button onClick={() => setVista('laboratorio')} class="px-3 py-1.5 text-[11px] font-bold rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                    <i class="fa-solid fa-vial-circle-check mr-1"></i>{t('tab_lab')}
                                </button>
                                <button onClick={() => setVista('metodologia')} class="px-3 py-1.5 text-[11px] font-bold rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                    <i class="fa-solid fa-book-open mr-1"></i>{t('tab_methodology')}
                                </button>
                                <button onClick={() => setVista('bibliografia')} class="px-3 py-1.5 text-[11px] font-bold rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                    <i class="fa-solid fa-quote-right mr-1"></i>{t('tab_bibliography')}
                                </button>
                            </div>
                        </div>
