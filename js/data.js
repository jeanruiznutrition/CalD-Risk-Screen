...
const ALIMENTOS_INICIALES = [
    ...
    },
    {
        ...
        grupoKey: 'group_animal',
        calcioPorcion: 276, faAlimento: 0.270, cargaReferencia: 325,
        vitDPorcion: 10.6, formaVitD: 'D3',
        proteinaPorcion: 19,
        gramosPorcion: 85,
        porcionUnidadKey: 'unit_3oz',
    ...
    },
    {
        ...
        grupoKey: 'group_animal',
        calcioPorcion: 13, faAlimento: 0.270, cargaReferencia: 300,
        vitDPorcion: 8.5, formaVitD: 'D3',
        proteinaPorcion: 19,
        gramosPorcion: 85,
        porcionUnidadKey: 'unit_3oz',
        icono: 'fa-solid fa-fish',
    ...
    },
    {
        ...
        grupoKey: 'group_animal',
        calcioPorcion: 13, faAlimento: 0.321, cargaReferencia: 300,
    ...
    },
    {
        ...
        grupoKey: 'group_grains',
        calcioPorcion: 220, faAlimento: 0.240, cargaReferencia: 300,
        vitDPorcion: 1.5, formaVitD: 'D2',
        proteinaPorcion: 3,
        gramosPorcion: 35,
        porcionUnidadKey: 'unit_one_cup',
        icono: 'fa-solid fa-bowl-rice',
        maxPorciones: 2,
        advertenciaKey: 'warn_check_label_vitd',
        fuenteKey: 'src_label_panama',
        // El dato menos fiable del catálogo: la fortificación varía mucho
        // entre marcas y una taza no es una masa fija (unos 30 g de
        // hojuelas frente a más de 50 g de cereal denso). Se marca por
        // omisión como valor genérico para que el evaluador lo verifique
        // con la etiqueta, y esa procedencia viaja al CSV.
        exigeVerificarEtiqueta: true
...
// ============================================================
// 20. (RETIRADO) SARC-CalF Y CIRCUNFERENCIA DE PANTORRILLA
// ============================================================
// Retirado en la v6.0 por decisión del autor. El SARC-F y el SARC-CalF
// son instrumentos validados por terceros y no son aporte de esta
// herramienta; no sirven a su propósito y el SARC-F se derivó en adultos
// mayores, de modo que aplicarlo a participantes jóvenes lo usaba fuera
// de su población de derivación. La proteína, que sí pertenece a esta
// herramienta, se conserva para todas las edades.
...
// ============================================================
// 23. CRITERIOS DE LAS RECOMENDACIONES
// ============================================================
// POR QUÉ ESTOS CRITERIOS Y NO OTROS
//
// Recomendar una prueba es una decisión clínica, y una herramienta que la
// emita tiene que declarar con qué criterio. Los de EDAD Y SEXO para la
// densitometría se anclan a la guía publicada (grado CONSENSO): mujeres
// de 65 años o más y varones de 70 o más, y entre 50 y 69 cuando hay
// factores de riesgo. Solo el componente de "riesgo calculado" es
// heurístico, y como tal está en el registro de parámetros.
//
// LO QUE DELIBERADAMENTE NO ES UN CRITERIO: EL PATRÓN DIETÉTICO
//
// No hay regla "vegano → densitometría", y la omisión es intencionada.
// En EPIC-Oxford (Tong et al., BMC Med 2020;18:353) el exceso de riesgo
// de fractura en veganos se atenuaba cuando la ingesta de calcio y
// proteína era adecuada: no es el patrón alimentario lo que constituye el
// riesgo, sino no cubrir el requerimiento. El patrón entra por donde
// debe, a través de la estimación de ingesta, y no como etiqueta.
// Recomendar densitometría por ser vegano estigmatizaría el patrón y
// afirmaría lo que el estudio de validación está diseñado para averiguar.
const DXA_EDAD_MUJER = 65;              // consenso de guía
const DXA_EDAD_VARON = 70;              // consenso de guía
const DXA_EDAD_CON_FACTORES = 50;       // consenso: 50-69 con factores de riesgo
// Fracción del máximo del puntaje óseo por encima de la cual se sugiere
// densitometría en el tramo de 50 a 69 años. HEURÍSTICO: es uno de los
// parámetros que el estudio debe calibrar contra el T-score.
const DXA_FRACCION_RIESGO = 0.40;

// Criterios para sugerir 25-hidroxivitamina D sérica. El primero no es
// una elección: es el caso en que la estimación NO puede concluir, porque
// la ingesta queda bajo la referencia pero la síntesis cutánea es
// apreciable, y la ingesta de referencia se definió suponiendo exposición
// solar mínima. Los demás son de cribado.
const VITD_LAB_FRACCION_INGESTA = 0.70;   // heurístico: ingesta bajo el 70% de la referencia
const VITD_LAB_FRACCION_RIESGO_OSEO = 0.40; // heurístico: riesgo óseo moderado o alto
...
const REGISTRO_PARAMETROS = [
    ...
    { clave: 'DXA_EDAD_MUJER', valor: DXA_EDAD_MUJER, unidad: 'años', grado: 'consenso',
      fuente: 'Guía de cribado de densidad mineral ósea: densitometría en mujeres de 65 años o más' },
    { clave: 'DXA_EDAD_VARON', valor: DXA_EDAD_VARON, unidad: 'años', grado: 'consenso',
      fuente: 'Guía de cribado de densidad mineral ósea: densitometría en varones de 70 años o más' },
    { clave: 'DXA_EDAD_CON_FACTORES', valor: DXA_EDAD_CON_FACTORES, unidad: 'años', grado: 'consenso',
      fuente: 'Guía de cribado: entre 50 y 69 años, densitometría si hay factores de riesgo' },
    { clave: 'DXA_FRACCION_RIESGO', valor: DXA_FRACCION_RIESGO, unidad: 'fracción del puntaje máximo', grado: 'heuristico',
      fuente: 'Corte propio del puntaje óseo para sugerir densitometría entre 50 y 69 años. Pendiente de calibrar contra el T-score' },
    { clave: 'VITD_LAB_FRACCION_INGESTA', valor: VITD_LAB_FRACCION_INGESTA, unidad: 'fracción de la referencia', grado: 'heuristico',
      fuente: 'Corte de cribado propio para sugerir 25-hidroxivitamina D sérica. Pendiente de calibración' },
    { clave: 'VITD_LAB_FRACCION_RIESGO_OSEO', valor: VITD_LAB_FRACCION_RIESGO_OSEO, unidad: 'fracción del puntaje máximo', grado: 'heuristico',
      fuente: 'Corte de cribado propio del riesgo óseo para sugerir 25-hidroxivitamina D sérica. Pendiente de calibración' }
];
