...
const {
    ...
    resumirBioquimicaParaRiesgoOseo, generarConductaSugerida,
    evaluarDistribucion, generarRecomendaciones, calcularRiesgoOseoSimple,
    REGISTRO_PARAMETROS, CARDA_VERSION,
    ...
    DIAAS_POR_ALIMENTO, TIPOS_AGUA, MULTIPLICADOR_VITD_POR_IMC
} = contexto;
...
// ============================================================
bloque('10. Objetivos de proteína');
// Las pruebas del SARC-F y del SARC-CalF se retiraron junto con el módulo:
// son instrumentos validados por terceros, no son aporte de esta
// herramienta y el SARC-F se derivó en adultos mayores. Las de proteína
// se conservan, porque la proteína sí pertenece a esta herramienta.
...
bloque('25. (Retirado) SARC-CalF');
// Retirado con el módulo de sarcopenia.
...
// ============================================================
// 37. PORCIONES DE CARNE Y PESCADO EN 3 ONZAS
// ============================================================
// Cambio de la v6.0 a petición del autor: 3 onzas (85 g) es la medida de
// uso en Panamá. Los valores se reescalaron desde los 100 g anteriores,
// así que esta prueba fija la coherencia entre la masa declarada y el
// contenido: si alguien cambia uno sin el otro, falla.

bloque('37. Porciones de carne y pescado en 3 onzas');

const FUENTE_3OZ = 'Medida de uso en Panamá: 3 onzas = 85 g. Los valores por porción se reescalaron proporcionalmente desde los de 100 g';

['carne_pollo', 'pescado_sin_espina', 'pescado_con_espina'].forEach(id => {
    const a = ALIMENTOS_INICIALES.find(x => x.id === id);
    verificarIgual(`La porción de ${id} es de 85 g`, a.gramosPorcion, 85, FUENTE_3OZ);
    verificarIgual(`Y su unidad declarada es 3 onzas`, a.porcionUnidadKey, 'unit_3oz', FUENTE_3OZ);
});

// El pescado con espina sigue siendo la fuente de calcio animal más
// densa del catálogo, que es la razón por la que está en él.
const pescadoEspina = ALIMENTOS_INICIALES.find(x => x.id === 'pescado_con_espina');
const pescadoSinEspina = ALIMENTOS_INICIALES.find(x => x.id === 'pescado_sin_espina');
verificarCierto('El pescado con espina aporta mucho más calcio que el filete sin espina',
    pescadoEspina.calcioPorcion > pescadoSinEspina.calcioPorcion * 10,
    'El calcio del pescado en conserva está en la espina blanda, no en el músculo');

const cerealFort = ALIMENTOS_INICIALES.find(x => x.id === 'cereal_fortificado');
verificarIgual('El cereal fortificado se declara por taza', cerealFort.porcionUnidadKey, 'unit_one_cup',
    'Petición del autor; una taza de hojuelas pesa unos 35 g');
verificarIgual('Y viene marcado para verificar con la etiqueta',
    cerealFort.exigeVerificarEtiqueta, true,
    'Es el dato menos fiable del catálogo: la fortificación varía mucho entre marcas y una taza no es una masa fija');


// ============================================================
// 38. DISTRIBUCIÓN DE LAS FUENTES A LO LARGO DE LA SEMANA
// ============================================================
// La métrica existe para enseñar al participante a repartir mejor sus
// fuentes. Su valor depende de que el efecto que dice medir sea real, y
// eso es lo que esta prueba comprueba: con el MISMO calcio declarado,
// repartirlo en más comidas tiene que rendir más calcio absorbido.

bloque('38. Distribución de las fuentes');

const FUENTE_DISTRIBUCION = 'Heaney RP, Weaver CM, Fitzsimmons ML. J Bone Miner Res 1990;5:1135 — la absorción fraccional decrece con la carga, así que repartir la misma cantidad en más comidas rinde más';

const refDist = obtenerReferenciaCalcio(35, 'femenino', 'IOM');
const baseDist = (veces, porciones) => ALIMENTOS_INICIALES.map(a => ({
    ...a,
    diasPorSemana: a.id === 'bebida_veg_fortificada' ? 7 : 0,
    vecesPorDia: veces, porcionesPorComida: porciones
}));
const repartido = ejecutarSemanaVirtualCalcio(baseDist(2, 1), null, {}, refDist);
const concentrado = ejecutarSemanaVirtualCalcio(baseDist(1, 2), null, {}, refDist);

verificarIgual('Las dos formas declaran el mismo calcio ingerido',
    Math.round(repartido.promedioIngeridoSemanal), Math.round(concentrado.promedioIngeridoSemanal),
    'La comparación solo tiene sentido si el total es el mismo');
verificarCierto('Repartir el mismo calcio en dos comidas rinde más absorbido que concentrarlo en una',
    repartido.promedioAbsorbidoSemanal > concentrado.promedioAbsorbidoSemanal, FUENTE_DISTRIBUCION);

const distRepartida = evaluarDistribucion(repartido);
const distConcentrada = evaluarDistribucion(concentrado);
verificarCierto('La métrica detecta que la versión concentrada está peor distribuida',
    distConcentrada.concentracionMayorComida > distRepartida.concentracionMayorComida,
    FUENTE_DISTRIBUCION);
verificarCierto('Y le atribuye un margen de mejora mayor por redistribución',
    distConcentrada.margenRedistribucionPuntos > distRepartida.margenRedistribucionPuntos,
    'El margen se estima con la propia curva: es lo que se ganaría sin cambiar nada de lo que se come');
verificarIgual('La salida recuerda que es una reconstrucción y no un registro de días',
    distRepartida.notaKey, 'dist_is_a_reconstruction',
    'La semana virtual reparte las frecuencias declaradas; no son los días reales del participante');
verificarIgual('Los días evaluados son siempre siete', distRepartida.diasTotales, 7,
    'El denominador tiene que ser explícito para que la cifra se pueda interpretar');


// ============================================================
// 39. RECOMENDACIONES: DERIVACIÓN, LABORATORIO Y DENSITOMETRÍA
// ============================================================

bloque('39. Recomendaciones');

const FUENTE_EPIC_VEGANOS = 'Tong TYN et al. (EPIC-Oxford) BMC Med 2020;18:353 — el exceso de riesgo de fractura en veganos se atenuaba con ingesta adecuada de calcio y proteína: el riesgo lo determina no cubrir el requerimiento, no la etiqueta dietética';

const conductaBrecha = generarConductaSugerida({
    razonCalcioNeta: 55, resultadoVitDDieta: { totalEq: 3, meta: 15 },
    resultadoSolar: { uiPromedioDia: 0 }
});
const oseoAlto = calcularRiesgoOseoV6({
    porcentajeCumplimientoCalcio: 55, categoriaRiesgoSolar: 'alto', categoriaVitDDieta: 'baja',
    edad: 58, sexo: 'femenino', diasEjercicioFuerza: 0, fuma: true, alcoholFrecuente: false,
    bajoUmbralEpicOxford: true, esVegano: true
}, null);

const recVegana58 = generarRecomendaciones({
    edad: 58, sexo: 'femenino', grupoEstudio: 'Vegano',
    conducta: conductaBrecha, resultadoOseo: oseoAlto, fuma: true
});
verificarIgual('A un participante con patrón basado en plantas se le deriva a nutricionista con formación en ese campo',
    recVegana58.derivacion.profesionalKey, 'refer_plant_based_dietitian',
    'Razón de competencia: biodisponibilidad del calcio vegetal, fortificación de bebidas vegetales y manejo de la vitamina D sin fuentes animales');
verificarCierto('Y con brecha de ingesta se sugiere 25-hidroxivitamina D sérica',
    recVegana58.solicitar25OHD,
    'Antes de suplementar conviene saber dónde está el participante');
verificarCierto('Con 58 años y riesgo elevado se sugiere densitometría',
    recVegana58.solicitarDXA,
    'Guía de cribado: entre 50 y 69 años, densitometría si hay factores de riesgo');

// --- LA PRUEBA QUE FIJA LA DECISIÓN DE DISEÑO MÁS IMPORTANTE ---
// No existe regla "vegano -> densitometría". Un participante joven con
// patrón vegano y requerimientos cubiertos no debe recibir ninguna
// recomendación de prueba: recomendársela por su etiqueta dietética
// estigmatizaría el patrón y sería gasto sin beneficio.
const conductaCubierta = generarConductaSugerida({
    razonCalcioNeta: 115, resultadoVitDDieta: { totalEq: 18, meta: 15 },
    resultadoSolar: { uiPromedioDia: 400 }
});
const oseoBajo = calcularRiesgoOseoV6({
    porcentajeCumplimientoCalcio: 115, categoriaRiesgoSolar: 'bajo', categoriaVitDDieta: 'adecuada',
    edad: 28, sexo: 'femenino', diasEjercicioFuerza: 3
}, null);
const recVegana28 = generarRecomendaciones({
    edad: 28, sexo: 'femenino', grupoEstudio: 'Vegano',
    conducta: conductaCubierta, resultadoOseo: oseoBajo
});
verificarIgual('A una vegana joven con requerimientos cubiertos NO se le recomienda densitometría',
    recVegana28.solicitarDXA, false, FUENTE_EPIC_VEGANOS);
verificarIgual('Ni se le solicita 25-hidroxivitamina D',
    recVegana28.solicitar25OHD, false,
    'Pedir una prueba a quien no la necesita es gasto sin beneficio y resta credibilidad al instrumento');
verificarIgual('Ni se la deriva', recVegana28.derivacion.necesaria, false,
    'Confirmar que alguien está bien es un resultado del instrumento');
verificarIgual('La salida declara que el patrón dietético no es criterio de densitometría',
    recVegana28.notaPatronDietetico, 'dxa_no_diet_criterion',
    'Es una omisión deliberada y un revisor va a preguntar por ella');

// La edad sola sí basta, por guía publicada
const recMujer67 = generarRecomendaciones({
    edad: 67, sexo: 'femenino', grupoEstudio: 'Omnívoro',
    conducta: conductaCubierta, resultadoOseo: oseoBajo
});
verificarCierto('A una mujer de 67 años se le recomienda densitometría por edad, cubra o no el requerimiento',
    recMujer67.solicitarDXA && recMujer67.motivosDXA.indexOf('dxa_age_women') >= 0,
    'Guía de cribado: mujeres de 65 años o más');
const recVaron67 = generarRecomendaciones({
    edad: 67, sexo: 'masculino', grupoEstudio: 'Omnívoro',
    conducta: conductaCubierta, resultadoOseo: oseoBajo
});
verificarIgual('A un varón de 67 años, en cambio, todavía no: el corte por guía son los 70',
    recVaron67.motivosDXA.indexOf('dxa_age_men'), -1,
    'Los cortes por sexo son distintos en la guía y la herramienta los respeta');

// No se pide una prueba que ya se tiene
const panelConD = evaluarPanelOseo({
    vitD25OH: 22, calcioSerico: 9.2, albumina: 4.1, edad: 40, sexo: 'femenino', pesoKg: 62
});
const recConD = generarRecomendaciones({
    edad: 40, sexo: 'femenino', grupoEstudio: 'Vegano',
    conducta: generarConductaSugerida({
        razonCalcioNeta: 60, resultadoVitDDieta: { totalEq: 3, meta: 15 },
        resultadoSolar: { uiPromedioDia: 0 }, panelBioquimico: panelConD
    }),
    resultadoOseo: oseoBajo, panelBioquimico: panelConD
});
verificarIgual('Si la 25-hidroxivitamina D ya está declarada, no se vuelve a solicitar',
    recConD.solicitar25OHD, false,
    'Recomendar una prueba que el participante ya trae delata que la herramienta no lee sus propios datos');

// Toda clave de recomendación tiene que estar traducida
const clavesRec = new Set();
[recVegana58, recVegana28, recMujer67, recConD].forEach(r => {
    clavesRec.add(r.derivacion.profesionalKey);
    clavesRec.add(r.descargoKey);
    clavesRec.add(r.notaPatronDietetico);
    if (r.derivacion.requiereMedicoKey) clavesRec.add(r.derivacion.requiereMedicoKey);
    r.motivos25OHD.forEach(k => clavesRec.add(k));
    r.motivosDXA.forEach(k => clavesRec.add(k));
});
const traduccionRec = (new Function(
    fs.readFileSync(path.join(raizJs, 'i18n', 'es.js'), 'utf8') + '; return TRADUCCION_ES;'
))();
verificarIgual('Todas las claves de las recomendaciones están traducidas',
    [...clavesRec].filter(k => !traduccionRec[k]).length, 0,
    'Sin la clave, la interfaz mostraría el identificador interno al paciente');



// ============================================================
// 40. PUNTAJE ÓSEO SIMPLE FRENTE AL COMPLETO
// ============================================================

bloque('40. Puntaje óseo simple frente al completo');

const FUENTE_DOS_PUNTAJES = 'Los dos puntajes existen para que el estudio determine empíricamente si los componentes no nutricionales aportan capacidad predictiva sobre la densitometría, en vez de decidirlo por doctrina';

const entradaMala = { porcentajeCumplimientoCalcio: 40, categoriaRiesgoSolar: 'alto', categoriaVitDDieta: 'baja' };
const entradaBuena = { porcentajeCumplimientoCalcio: 120, categoriaRiesgoSolar: 'bajo', categoriaVitDDieta: 'adecuada' };

verificarIgual('El puntaje simple usa solo cuatro puntos posibles',
    calcularRiesgoOseoSimple(entradaBuena).puntajeMaximo, 4,
    'Dos por calcio y dos por vitamina D: son los dos componentes que la herramienta estima con su propio modelo');
verificarIgual('Adecuación plena da puntaje cero',
    calcularRiesgoOseoSimple(entradaBuena).puntaje, 0, FUENTE_DOS_PUNTAJES);
verificarIgual('Déficit en ambos nutrientes agota el puntaje simple',
    calcularRiesgoOseoSimple(entradaMala).puntaje, 4, FUENTE_DOS_PUNTAJES);

// El simple NO debe moverse con variables que no usa: es la propiedad que
// hace interpretable la comparación del estudio.
const simpleJoven = calcularRiesgoOseoSimple(entradaMala);
const completoJoven = calcularRiesgoOseoV6({ ...entradaMala, edad: 30, sexo: 'femenino', diasEjercicioFuerza: 3 }, null);
const completoMayor = calcularRiesgoOseoV6({ ...entradaMala, edad: 70, sexo: 'femenino', diasEjercicioFuerza: 0, fuma: true }, null);
verificarIgual('El puntaje simple no cambia con la edad, el sexo ni el ejercicio',
    simpleJoven.puntaje, calcularRiesgoOseoSimple({ ...entradaMala, edad: 70, fuma: true }).puntaje,
    'Si se moviera con esas variables, la comparación contra el completo no aislaría su contribución');
verificarCierto('El completo sí sube con edad, tabaquismo e inactividad',
    completoMayor.puntaje > completoJoven.puntaje,
    'Son determinantes documentados de la masa ósea; la pregunta es si aportan sobre lo que ya aporta la nutrición');
verificarCierto('El puntaje simple declara explícitamente qué excluye',
    simpleJoven.excluye.indexOf('actividad_fisica') >= 0 && simpleJoven.excluye.indexOf('edad') >= 0,
    'El análisis tiene que poder saber qué se dejó fuera sin leer el código');
