...
verificarCierto('El tofu sedoso aporta un calcio absorbible despreciable frente al cuajado con calcio',
    calcularCalcioAbsorbidoItem(47, alimento('tofu_sedoso').faAlimento, 47, alimento('tofu_sedoso').cargaReferencia)
    < 0.2 * calcularCalcioAbsorbidoItem(525, alimento('tofu_sulfato_calcio').faAlimento, 525, alimento('tofu_sulfato_calcio').cargaReferencia),
    'USDA FoodData Central: sulfato de calcio 350-683 mg/100 g vs GDL ~31 mg/100 g');
...
const dietaPobre = ejecutarSemanaVirtualCalcio(
    [construir('verduras_alto_oxalato', 7, 2), construir('tofu_sedoso', 5), construir('almendras', 7)],
    null, {}, ref);
...
// ============================================================
// 11. FRAX: CAPTURA, NO CÁLCULO
// ============================================================
bloque('11. FRAX: la herramienta captura datos, no calcula probabilidad');

const frax = prepararDatosFRAX({
    edad: 62, sexo: 'femenino', pesoKg: 62, tallaCm: 160,
    factores: { fracturaPrevia: true, fumadorActual: true },
    paisSustituto: 'ecuador'
});

verificar('El IMC se calcula correctamente', frax.imc, 24.2, 0.02, 'peso / talla²');
verificarIgual('Se contabilizan los factores de riesgo presentes', frax.numeroFactores, 2, 'FRAX: 11 factores clínicos');
verificarIgual('Se detecta que la edad está en el rango de validez de FRAX', frax.edadEnRango, true, 'FRAX válido de 40 a 90 años');
verificarIgual('Se marca la edad fuera de rango cuando corresponde',
    prepararDatosFRAX({ edad: 25, sexo: 'femenino', pesoKg: 60, tallaCm: 165, factores: {} }).edadEnRango,
    false, 'FRAX válido de 40 a 90 años');
verificarCierto('Se señala que se requiere un modelo nacional sustituto',
    frax.requiereModeloSustituto === true,
    'Panamá no dispone de modelo FRAX calibrado');
verificarCierto('No existe ninguna función que calcule la probabilidad FRAX',
    typeof contexto.calcularFRAX === 'undefined' && typeof contexto.calcularProbabilidadFractura === 'undefined',
    'FRAX es propietario (Universidad de Sheffield); su automatización requiere licencia');
